...
| MoonClaw agent sessions | Working | The Agents activity lists per-book sessions, starts or connects to the MoonClaw daemon through a native MoonClaw executable, creates/reuses the book-scoped task for the workspace root, sends contextual user messages, folds saved MoonClaw assistant/tool/failure events and workspace `.moonclaw/log.jsonl` progress rows into the transcript, supports web-search/model selection, cancels active task work, and persists local session metadata under `.moontown/moondesk-agent-sessions/`. The MoonCode protocol view now knows about MoonClaw's native `runtime-turn` endpoint for claim -> explicit/deterministic/model-planned tool execution -> runtime-completed/runtime-failed receipts, native `runtime-loop` queue supervision for run-until-idle/failure/cancel/max-turns execution, and daemon-owned `runtime-service` supervision with `runtime.service_started`/`runtime.service_finished` lifecycle events; Moondesk exposes that service through `POST /api/mooncode/sessions/<id>/runtime-service`, `Start Service`, action-plan `Start Service` recommendations for pending runtime/proof work, lifecycle status projection, and an explicit `Stop Service` action that queues a MoonCode cancel command through the ordered runtime stream. The session summary now carries a product-grade `production_score`/`production_level` checklist over the end-to-end MoonCode user workflow, separate from eval harness readiness, and the score now requires a current action-plan manifest with no blocked actions, pending diffs/tool approvals/steers, or failing tests before it can reach `production-grade`. The stack also separates command-scoped runtime proof from unscoped live transcript/reasoning/tool events in the Runtime Events panel, supports native `steer_applied`/`steer_deferred`/`steer_dropped` settlement evidence for steering commands, renders row-level `steering_lifecycle` for applied/deferred/dropped/pending steering, row-level `cancel_lifecycle` for cancelled/dropped/failed/pending cancellation, and header-level `patch_lifecycle` for needs-review/runtime-needed/runtime-proven/blocked file changes directly in the MoonCode session header. It also supports native `receipt.accept`/`receipt.reject` review receipts for runtime-consumed accept/reject commands, native package manifest/index creation with `package_built`/`package_verified` proof for generated tools and miniapps, source-promoted package manifests under `portable/app-tool/mooncode/<session>/sources/<command>/...`, native bounded `apply_patch`/`revert_patch` tools for reviewed text replacements plus single-file or multi-file unified-diff patchsets with `runtime.patch_applied`/`runtime.patch_reverted` proof events, selected-hunk dispatch through `hunk_index`/`hunk_id` or `path#hunk-N` targets with `hunk_dispatch_scope` and `file_path` proof metadata, optional post-change patch verification metadata from `verification_command`/`test_command`/`verify_after`/`moon_check_target`, and MoonClaw's first native eval harness report from the selected book-scoped eval endpoint. Native eval readiness now requires `tool_harness`, `file_edit`, `patch_review`, `command_execution`, and `package_output` proof, so old tool/file-edit-only reports remain visible but partial. MoonClaw can now use an explicitly selected model for bounded OpenSeek-style tool-call planning with tool-result feedback, deterministic fallback, and native reasoning/assistant/tool-call/tool-result events; the remaining gap is deeper resume orchestration, diff-review polish, binary/bundle assembly polish, and broader model-backed coding eval coverage. |
