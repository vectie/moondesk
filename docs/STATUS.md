...
| Area | Status | Notes |
| --- | --- | --- |
| Workspace discovery and Desk | Working | Discovers MoonBooks under `books`, exposes a Moontown root only when `.moontown/` exists, projects MoonBook virtual sections at the root, lists scoped entries with file metadata, shows directory breadcrumbs, entry counts, selection/access state, and opens common book paths. Plain folders are not advertised as workspaces. |
