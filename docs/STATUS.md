...
- Reconcile Moondesk-managed daemon state with any independently running
  Moontown daemon.
- Finish the MoonClaw autonomous MoonCode loop beyond the current queue
  supervisor and one-batch model planner: persistent multi-step tool choice,
  resume/steer semantics, and model-backed coding eval proof.
