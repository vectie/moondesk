...
- Reconcile Moondesk-managed daemon state with any independently running
  Moontown daemon.
- Finish the MoonClaw autonomous MoonCode loop that chooses tools from prompts,
  resumes/steers multi-turn coding work, and expands the first native
  tool/file-edit harness into model-backed coding eval proof.
